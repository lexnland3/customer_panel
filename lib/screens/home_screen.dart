...
class _HomeTabState extends State<HomeTab> {
  ...
  final _facings   = ['Any','North','South','East','West','North-East','North-West','South-East','South-West'];
  final _plotTypes = ['Any','Agricultural','Residential','Commercial','Industrial'];

  @override
  void initState() { super.initState(); _load(); _loadFavIds(); }

  Future<void> _loadFavIds() async {
    try {
      final res = await Api.getFavouriteIds();
      if (mounted) setState(() => _favIds = Set<String>.from(res['ids'] as List? ?? []));
    } catch (_) {}
  }
  ...
  Future<void> _toggleFav(String plotId) async {
    ...
    setState(() { wasFav ? _favIds.remove(plotId) : _favIds.add(plotId); });
    try {
      await Api.toggleFavourite(plotId, wasFav);
    } catch (_) {
      if (mounted) setState(() { wasFav ? _favIds.add(plotId) : _favIds.remove(plotId); });
    }
  }
          ...
          slivers: [
            ...
            SliverToBoxAdapter(
              child: Container(
                ...
                child: Column(children: [
                  Row(children: [
                    const Icon(Icons.location_on_rounded, color: C.primary, size: 18),
                    const SizedBox(width: 4),
                    const Expanded(child: Text('Amritsar, Punjab',
                        style: TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: C.textDark))),
                    IconButton(
                      icon: const Icon(Icons.notifications_outlined, color: C.textDark, size: 22),
                      onPressed: () {},
                      padding: EdgeInsets.zero, constraints: const BoxConstraints(),
                    ),
                    const SizedBox(width: 12),
                    IconButton(
                      icon: const Icon(Icons.chat_bubble_outline_rounded, color: C.textDark, size: 22),
                      onPressed: () {},
                      padding: EdgeInsets.zero, constraints: const BoxConstraints(),
                    ),
                  ]),
                ...
                child: Stack(children: [
                  ...
                  Positioned(right: 16, bottom: 0,
                    child: Opacity(opacity: 0.18,
                      child: Icon(Icons.landscape_rounded, size: 140, color: Colors.white))),
                  // Content
                  Padding(
                    padding: const EdgeInsets.all(20),
                    child: Column(crossAxisAlignment: CrossAxisAlignment.start,
                        mainAxisAlignment: MainAxisAlignment.center, children: [
                      const Text('Find your', style: TextStyle(fontSize: 14, color: Colors.white70)),
                        ...
                        child: Container(
                          ...
                          child: const Row(mainAxisSize: MainAxisSize.min, children: [
                            Text('Explore Now', style: TextStyle(
                                fontSize: 13, fontWeight: FontWeight.w800, color: C.primary)),
                            SizedBox(width: 4),
              ...
              SliverToBoxAdapter(
                child: SizedBox(
                  ...
                  child: ListView(
                    ...
                    children: [
                      if (_facing != 'Any')   _FilterPill('Facing: $_facing',   () { setState(() => _facing   = 'Any'); _load(); }),
                      if (_plotType != 'Any') _FilterPill('Type: $_plotType',   () { setState(() => _plotType = 'Any'); _load(); }),
                    ],
                  ),
                ),
              ),
            ...
            SliverToBoxAdapter(
              child: Padding(
                ...
                child: Row(children: [
                  const Expanded(child: Text('Near By Your location',
            ...
            if (_loading)
              ...
              SliverFillRemaining(child: _Empty(onReset: () {
                _searchCtrl.clear();
                setState(() { _facing = 'Any'; _plotType = 'Any'; });
                _load();
              }))
            else
              SliverList(
      ...
      builder: (_) => StatefulBuilder(builder: (ctx, ss) => Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            const Text('Filter Plots', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w800, color: C.textDark)),
            TextButton(onPressed: () => ss(() { tf = 'Any'; tp = 'Any'; }),
                child: const Text('Reset', style: TextStyle(color: C.primary))),
          ]),
          const SizedBox(height: 16),
          const Text('Plot Type', style: TextStyle(fontWeight: FontWeight.w700, color: C.textDark)),
          const SizedBox(height: 8),
          Wrap(spacing: 8, runSpacing: 6, children: _plotTypes.map((t) => ChoiceChip(
            label: Text(t), selected: tp == t,
            onSelected: (_) => ss(() => tp = t),
            selectedColor: C.primaryLight,
            labelStyle: TextStyle(color: tp == t ? C.primary : C.textMuted, fontWeight: FontWeight.w600),
          )).toList()),
          const SizedBox(height: 16),
          const Text('Facing', style: TextStyle(fontWeight: FontWeight.w700, color: C.textDark)),
          const SizedBox(height: 8),
          Wrap(spacing: 8, runSpacing: 6, children: _facings.map((f) => ChoiceChip(
            label: Text(f), selected: tf == f,
            onSelected: (_) => ss(() => tf = f),
            selectedColor: C.primaryLight,
            labelStyle: TextStyle(color: tf == f ? C.primary : C.textMuted, fontWeight: FontWeight.w600),
          )).toList()),
          const SizedBox(height: 20),
          SizedBox(width: double.infinity, child: ElevatedButton(
            onPressed: () {
              setState(() { _facing = tf; _plotType = tp; });
              Navigator.pop(ctx);
              _load();
            },
...
class _PlotCard extends StatelessWidget {
  ...
  const _PlotCard({required this.plot, required this.isFav, required this.onFavTap, required this.onTap});
              ...
              Row(children: [
                ...
                if (isVerified)
                  Row(mainAxisSize: MainAxisSize.min, children: const [
                    Icon(Icons.star_rounded, color: Color(0xFFFFA726), size: 14),
                    SizedBox(width: 2),
                    Text('4.9', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w700, color: Color(0xFFFFA726))),
                  ]),
              ]),
              const SizedBox(height: 3),
              Row(children: [
                const Icon(Icons.location_on_outlined, size: 13, color: C.textMuted),
...
class _PhotoPlaceholder extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: const Center(child: Icon(Icons.landscape_rounded, size: 60, color: C.primary)),
  );
}
...
class _Chip extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    decoration: BoxDecoration(
      ...
      border: isMore ? Border.all(color: C.primary.withValues(alpha: 0.3)) : null,
    ),
  ...
  Widget build(BuildContext context) => GestureDetector(
    ...
    child: Container(
      ...
      child: Row(mainAxisSize: MainAxisSize.min, children: [
        Text(label, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: C.primary)),
        const SizedBox(width: 4),
...
class _Empty extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Center(
    child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      ...
      const Text('No plots found', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w700, color: C.textDark)),
      const SizedBox(height: 6),
      const Text('Try different filters', style: TextStyle(color: C.textMuted)),
      const SizedBox(height: 16),
      TextButton(onPressed: onReset,
          child: const Text('Clear Filters', style: TextStyle(color: C.primary, fontWeight: FontWeight.w700))),
    ]),
  );
}
