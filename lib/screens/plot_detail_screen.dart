...
class _PlotDetailScreenState extends State<PlotDetailScreen> {
  ...
  // Opens the property location in Google Maps — the owner's exact map link if
  // provided, otherwise a Google Maps search of the address text.
  Future<void> _openLocation() async {
    final mapLink = (_plot?['mapLink'] ?? '').toString().trim();
    final loc = (_plot?['location'] ?? '').toString().trim();
    String? url;
  ...
  Future<void> _load() async {
    try {
      ...
      if (mounted)
        setState(() {
          _plot = res['property'] as Map<String, dynamic>?;
          _loading = false;
        });
      _checkFav();
      _loadMyRating();
            ...
            child: Padding(
              ...
              child: Column(
                  ...
                  children: [
                    ...
                    _DetailGrid(items: [
                      if ((d['plotType'] ?? '').isNotEmpty)
                        _DetailItem('Type', d['plotType']!),
                      if ((d['facing'] ?? '').isNotEmpty)
                        _DetailItem('Facing', d['facing']!),
                      if ((d['ownershipType'] ?? '').isNotEmpty)
                        _DetailItem('Ownership', d['ownershipType']!),
                      if ((d['plotId'] ?? '').isNotEmpty)
                        _DetailItem('Plot ID', d['plotId']!),
                    ]),
                    const SizedBox(height: 16),
                    ...
                    Navigator.push(
                        ...
                        MaterialPageRoute(
                          builder: (_) => ChatScreen(
                            ...
                            ownerName:
                                (chat['owner'] as Map?)?['name'] as String? ??
                                    'Owner',
